...
async function auditWorkday(value) {
  ...
  const urls=[];let offset=0,reportedTotal=null,pages=0,exhausted=false,repeatedPage=false;const limit=50;const pageSignatures=new Set();
  while(pages<MAX_PAGES){
    ...
    const explicitTotal=nonNegativeOrNull(payload?.total);if(explicitTotal!==null)reportedTotal=explicitTotal;
    const signature=stableSignature(rows.map(row=>`${text(row?.bulletFields?.[0]||row?.jobReqId||row?.id)}|${text(row?.externalPath)}|${text(row?.title)}`));
    if(rows.length&&pageSignatures.has(signature)){repeatedPage=true;break;}if(rows.length)pageSignatures.add(signature);
    pages++;
    ...
    if(reportedTotal!==null&&offset>=reportedTotal){exhausted=true;break;}
    if(rows.length<limit){exhausted=true;break;}
  }
  const unique=uniqueUrls(urls);const official=reportedTotal!==null?reportedTotal:(exhausted?unique.length:null);
  const done=official!==null&&exhausted&&!repeatedPage&&unique.length>=official;
  if(!done)return incomplete('Workday pagination completeness was not proven',config.sourceUrl,official,unique,{pages,tenant:config.tenant,site:config.site,reported_total:reportedTotal,exhausted,repeated_page:repeatedPage,hit_page_cap:pages>=MAX_PAGES&&!exhausted});
  return complete('Workday official career API',config.sourceUrl,official,unique,{pages,tenant:config.tenant,site:config.site,reported_total:reportedTotal,exhausted});
}
...
async function auditSmartRecruiters(board) {
  ...
  const base=`https://api.smartrecruiters.com/v1/companies/${encodeURIComponent(board)}/postings`;let offset=0,reportedTotal=null,pages=0,exhausted=false,repeatedPage=false,invalidIds=0;const limit=100;const ids=[];const pageSignatures=new Set();
  while(pages<MAX_PAGES){
    const url=new URL(base);url.searchParams.set('limit',String(limit));url.searchParams.set('offset',String(offset));const payload=await fetchJson(url.toString());const rows=Array.isArray(payload?.content)?payload.content:[];
    const explicitTotal=nonNegativeOrNull(payload?.totalFound??payload?.total);if(explicitTotal!==null)reportedTotal=explicitTotal;
    const pageIds=rows.map(row=>text(row?.id)).filter(Boolean);invalidIds+=Math.max(0,rows.length-pageIds.length);
    const signature=stableSignature(pageIds.length?pageIds:rows.map(row=>`${text(row?.name||row?.title)}|${text(row?.refNumber)}`));if(rows.length&&pageSignatures.has(signature)){repeatedPage=true;break;}if(rows.length)pageSignatures.add(signature);
    ids.push(...pageIds);pages++;offset+=rows.length;
    if(!rows.length){exhausted=true;break;}
    if(reportedTotal!==null&&offset>=reportedTotal){exhausted=true;break;}
    if(rows.length<limit){exhausted=true;break;}
  }
  const uniqueIds=Array.from(new Set(ids));const official=reportedTotal!==null?reportedTotal:(exhausted?uniqueIds.length:null);
  const done=official!==null&&exhausted&&!repeatedPage&&invalidIds===0&&uniqueIds.length>=official;
  if(!done)return incomplete('SmartRecruiters pagination completeness was not proven',base,official,[],{board,pages,reported_total:reportedTotal,ids_collected:uniqueIds.length,invalid_ids:invalidIds,exhausted,repeated_page:repeatedPage,hit_page_cap:pages>=MAX_PAGES&&!exhausted});
  return complete('SmartRecruiters official postings API',base,official,[],{board,pages,posting_ids:uniqueIds.slice(0,5000),count_only:true,reported_total:reportedTotal,exhausted});
}
...
async function auditJibe(sourceUrl) {
  ...
  let page=1,reportedTotal=null,pages=0,exhausted=false,repeatedPage=false;const urls=[];const basePath=source.pathname.replace(/\/+$/,'');const pageSignatures=new Set();
  while(page<=MAX_PAGES){const u=new URL(api.toString());u.searchParams.set('page',String(page));const payload=await fetchJson(u.toString());const rows=Array.isArray(payload?.jobs)?payload.jobs:[];const explicitTotal=nonNegativeOrNull(payload?.totalCount);if(explicitTotal!==null)reportedTotal=explicitTotal;const tokens=rows.map(row=>{const data=row?.data??row??{};return text(data.slug||data.req_id||data.reqId||data.id);}).filter(Boolean);const signature=stableSignature(tokens.length?tokens:rows.map(row=>JSON.stringify(row).slice(0,300)));if(rows.length&&pageSignatures.has(signature)){repeatedPage=true;break;}if(rows.length)pageSignatures.add(signature);for(const token of tokens)urls.push(new URL(`${basePath}/${encodeURIComponent(token)}`,source.origin).toString());pages++;if(!rows.length){exhausted=true;break;}const unique=uniqueUrls(urls);if(reportedTotal!==null&&unique.length>=reportedTotal){exhausted=true;break;}page++;}
  const unique=uniqueUrls(urls);const official=reportedTotal!==null?reportedTotal:(exhausted?unique.length:null);const done=official!==null&&exhausted&&!repeatedPage&&unique.length>=official;
  if(!done)return incomplete('Jibe pagination completeness was not proven',api.toString(),official,unique,{pages,reported_total:reportedTotal,exhausted,repeated_page:repeatedPage,hit_page_cap:page>MAX_PAGES&&!exhausted});return complete('Jibe official career API',api.toString(),official,unique,{pages,reported_total:reportedTotal,exhausted});
}
...
async function auditAmentum(sourceUrl) {
  const base=new URL(sourceUrl);if(!/amentumcareers\.com$/i.test(base.hostname))return unsupported('Amentum audit requires amentumcareers.com',sourceUrl);let page=1,reportedTotal=null,pages=0,exhausted=false,repeatedPage=false;const urls=[];const pageSignatures=new Set();
  while(page<=MAX_PAGES){const u=new URL(sourceUrl);u.searchParams.set('page',String(page));const html=await fetchText(u.toString(),'text/html,application/xhtml+xml');if(reportedTotal===null)reportedTotal=parseAmentumTotal(html);const pageUrls=Array.from(html.matchAll(/<a\b[^>]*href=["']([^"']+)["']/gi),m=>normalizeUrl(new URL(decodeHtml(m[1]),u).toString())).filter(v=>v&&/amentumcareers\.com\/jobs\/(?!search(?:[/?#]|$))[^/?#]{5,}/i.test(v));const signature=stableSignature(uniqueUrls(pageUrls));if(pageUrls.length&&pageSignatures.has(signature)){repeatedPage=true;break;}if(pageUrls.length)pageSignatures.add(signature);urls.push(...pageUrls);pages++;const unique=uniqueUrls(urls);if(!pageUrls.length){exhausted=true;break;}if(reportedTotal!==null&&unique.length>=reportedTotal){exhausted=true;break;}page++;}
  const unique=uniqueUrls(urls);const official=reportedTotal!==null?reportedTotal:(exhausted?unique.length:null);const done=official!==null&&exhausted&&!repeatedPage&&unique.length>=official;
  if(!done)return incomplete('Amentum pagination completeness was not proven',sourceUrl,official,unique,{pages,reported_total:reportedTotal,exhausted,repeated_page:repeatedPage,hit_page_cap:page>MAX_PAGES&&!exhausted});return complete('Amentum official careers inventory',sourceUrl,official,unique,{pages,reported_total:reportedTotal,exhausted});
}
...
function stableSignature(values){return (Array.isArray(values)?values:[]).map(value=>String(value||'')).filter(Boolean).sort().join('\u001f');}
