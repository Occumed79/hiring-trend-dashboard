...
async function auditOfficialSource(entity, source) {
  ...
  try {
    if (provider === 'governmentjobs' || provider === 'neogov' || governmentJobsSlug(boardId, sourceUrl)) {
...
async function auditSmartRecruiters(board) {
  ...
  // The public list API gives authoritative count/IDs but does not guarantee a stable public posting URL field.
...
async function auditPersonio(board) {
  ...
  for(const host of [`${slug}.jobs.personio.de`,`${slug}.jobs.personio.com`]){const url=`https://${host}/xml`;try{const xml=await fetchText(url,'application/xml,text/xml;q=0.9,*/*;q=0.7');if(!/<position\b/i.test(xml))continue;const blocks=splitXmlBlocks(xml,'position');const urls=[];let invalid=0;for(const block of blocks){const id=text(readXmlTag(block,'id'));const explicit=normalizeUrl(readXmlTag(block,'url')||readXmlTag(block,'jobUrl')||readXmlTag(block,'careerUrl'));const jobUrl=explicit||(id?`https://${slug}.jobs.personio.de/job/${encodeURIComponent(id)}`:null);if(jobUrl)urls.push(jobUrl);else invalid++;}if(invalid)return incomplete('Personio XML contained positions without stable URLs',url,blocks.length,urls,{board:slug,invalid});return complete('Personio official XML feed',url,blocks.length,urls,{board:slug});}catch{} }
...
function governmentJobsSlug(board,url){const direct=sanitizeToken(board);if(direct&&direct!=='governmentjobs'&&direct!=='neogov'&&!direct.includes('http'))return direct;if(url)try{const u=new URL(url);if(/governmentjobs\.com$/i.test(u.hostname)){const m=u.pathname.match(/\/careers\/([^/?#]+)/i);if(m?.[1])return sanitizeToken(m[1]);}}catch{}return null;}
