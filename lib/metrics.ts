...
export async function getEntityMetrics(entityId: string) {
  ...
  const historyRows = history.map((row: any) => ({ date: row.snapshot_date, totalActive: Number(row.total_active || 0), newThisWeek: Number(row.new_this_week || 0), closedCount: Number(row.closed_count || 0) }));
  if (!historyRows.length || Number(historyRows[historyRows.length - 1]?.totalActive) !== totalNow) historyRows.push({ date: new Date().toISOString().slice(0, 10), totalActive: totalNow, newThisWeek, closedCount: 0 });
...
export async function getEntityRoleBreakdown(entityId: string) {
  ...
  for (const job of jobs) { const role = String(job.role_category || 'other'); result[role] = (result[role] || 0) + 1; }
  return result;
}
...
async function getSnapshotAtOrBefore(entityId: string, targetDate: string) {
  return query(`SELECT total_active FROM hiring_snapshots WHERE entity_id = $1 AND snapshot_date <= $2 AND snapshot_date >= $3::date ORDER BY snapshot_date DESC LIMIT 1`, [entityId, targetDate, QUALITY_BASELINE_DATE]);
}
