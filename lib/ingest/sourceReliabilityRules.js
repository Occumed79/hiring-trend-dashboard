...
function evaluateSourceReliability(input) {
  ...
  for (const check of checks) {
    ...
    const inventoryEvidence = sourceClass === 'authoritative' || sourceClass === 'verified';

    // Supplemental discovery sources keep their own ERROR status on their source
    // tile, but they do not create a profile-wide Reliability Diagnostics alarm.
    // Global incidents are reserved for sources that materially affect inventory
    // confidence: authoritative or verified evidence.
    if (status === 'error' && inventoryEvidence) {
    ...
    if (inventoryEvidence && (details.truncated === true || details.pagination_truncated === true)) {
    ...
    if (inventoryEvidence && previousCheck && isHealthy(status) && isHealthy(text(previousCheck.status))) {
