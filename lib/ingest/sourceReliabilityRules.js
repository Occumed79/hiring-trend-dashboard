...
function evaluateSourceReliability(input) {
  ...
  // Only compare inventories that can legitimately be compared. In particular,
  // an authoritative source returning zero without a complete-enumeration proof
  // is not a healthy zero and must never create a fake "0 vs N" disagreement.
...
function isComparableHealthy(check) {
  ...
  return text(check?.source_class) === 'authoritative' ? check?.authoritative_zero === true : true;
}
