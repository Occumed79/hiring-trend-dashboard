...
export function normalizeApplyUrl(item: any): string | null {
  ...
  const candidates = [item?.normalized_apply_url, item?.apply_url, item?.applyUrl, item?.url, item?.job_url, item?.job_apply_link,
    raw.normalized_apply_url, raw.absolute_url, raw.hostedUrl, raw.applyUrl, raw.apply_url, raw.url, raw.jobUrl, raw.job_url,
    raw.job_apply_link, raw.PositionURI, raw.positionURI, raw.canonicalPositionUrl, raw.ref];
  for (const candidate of candidates) { const normalized = normalizeUrl(candidate); if (normalized) return normalized; }
  return null;
}
...
export function dedupeJobsAcrossSources(items: any[]) {
  ...
  for (const item of items) {
    ...
    const identity = applyUrl ? `url:${applyUrl.toLowerCase()}` : source && externalId ? `source:${source}:${externalId}` : `row:${byIdentity.size}:${String(item.title || '').toLowerCase()}`;
    const normalizedItem = withNormalizedUrl(item, applyUrl);
    const existing = byIdentity.get(identity);
    if (!existing) byIdentity.set(identity, normalizedItem);
    else { duplicates++; byIdentity.set(identity, mergeDuplicateJobs(existing, normalizedItem)); }
  }
...
function filterByEmployerEvidence(items: any[], entity: any, shouldFilter: (source: string) => boolean) {
  ...
  for (const item of items) {
    const source = String(item?.source || '').trim().toLowerCase();
    if (!shouldFilter(source)) { jobs.push(item); continue; }
    const evidence = getEmployerEvidence(item, entity);
    if (!evidence) { rejected++; continue; }
    jobs.push({ ...item, raw_data: { ...(item.raw_data || {}), normalized_employer: entity?.name || item.raw_data?.normalized_employer || null, normalized_employer_match: evidence } });
  }
...
function getEmployerEvidence(item: any, entity: any): string | null {
  ...
  for (const name of names) {
    ...
    if (normalizedName.length >= 3 && containsPhrase(searchable, normalizedName)) return `employer-name:${name}`;
    const withoutLegalSuffix = stripLegalSuffix(normalizedName);
    if (withoutLegalSuffix.length >= 4 && withoutLegalSuffix !== normalizedName && containsPhrase(searchable, withoutLegalSuffix)) return `employer-name:${name}`;
  }
  return null;
}

function mergeDuplicateJobs(left: any, right: any) {
  const preferred = sourceScore(right?.source) > sourceScore(left?.source) ? right : left;
...
function withNormalizedUrl(item: any, normalizedApplyUrl: string | null) {
  return normalizedApplyUrl ? { ...item, raw_data: { ...(item.raw_data || {}), normalized_apply_url: normalizedApplyUrl } } : item;
}
...
function normalizeUrl(value: unknown): string | null {
  ...
  for (const key of Array.from(url.searchParams.keys())) {
    ...
    if (normalizedKey.startsWith('utm_') || TRACKING_PARAMS.has(normalizedKey)) url.searchParams.delete(key);
  }
  url.searchParams.sort();
  return url.toString();
}
function parseUrl(value: unknown): URL | null { if (!value) return null; try { const url = new URL(String(value).trim()); return ['http:','https:'].includes(url.protocol) ? url : null; } catch { return null; } }
