...
export function assessJobQuality(item: any): JobQualityResult {
  ...
  const hasStructuredEvidence = parser.includes('json_ld')
    || raw.normalized_employer_source
    || STRUCTURED_SOURCES.has(source)
    || source.startsWith('portal:')
    || source.startsWith('gov:');

  if (source === 'career_page' || source.startsWith('ats:')) {
    if (!hasStructuredEvidence && !strongDetailUrl) {
      return reject('career-page row lacks job-detail evidence', applyUrl, strongDetailUrl);
    }
  }

  if (LEGACY_WEAK_SOURCES.has(source) && !strongDetailUrl) {
    return reject('legacy discovery row lacks a direct job-detail URL', applyUrl, strongDetailUrl);
  }

  if ((source.startsWith('web:') || source.startsWith('jobapi:')) && !strongDetailUrl) {
    return reject('discovery row lacks a direct job-detail URL', applyUrl, strongDetailUrl);
  }
...
export function isLegacyWeakSource(source: unknown) {
  return LEGACY_WEAK_SOURCES.has(String(source || '').trim().toLowerCase());
}
...
function looksLikeMarkupOrCss(title: string) {
  return /[{}<>]|(?:^|\s)\.[a-z0-9_-]+\s*\{|display\s*:\s*(?:inline|block)|vertical-align\s*:/i.test(title)
    || /^(?:css|style|script)\b/i.test(title);
}
...
function reject(reason: string, applyUrl: string | null, strongDetailUrl: boolean): JobQualityResult {
  return { ok: false, reason, applyUrl, strongDetailUrl };
}
