...
export default function WorldMap({ entityId, portalId }: { entityId?: string; portalId?: string }) {
  ...
  useEffect(() => {
    ...
    const points = mapData.map((point: any) => [Number(point.lat), Number(point.lng)] as [number, number]).filter(([lat, lng]) => Number.isFinite(lat) && Number.isFinite(lng));
    if (!points.length) return;
    const frame = window.requestAnimationFrame(() => {
      const map = mapRef.current;
      if (!map) return;
      map.invalidateSize?.({ animate: false });
      if (points.length === 1) map.setView(points[0], 6, { animate: false });
      else map.fitBounds(MapComponents.L.latLngBounds(points).pad(0.14), { animate: false, maxZoom: 7, padding: [36, 36] });
    });
    return () => window.cancelAnimationFrame(frame);
  ...
  return (
    <div className={`map-glass-card h-full flex flex-col gap-3.5 ${profileMode ? 'min-h-[720px]' : 'min-h-[520px]'}`}>
        ...
        {!MapComponents ? <div className="w-full h-full bg-[#080d1a] rounded-xl animate-pulse" /> : (
          ...
          >
            ...
            {mapData.map((point: any, i: number) => {
              ...
              return (
                <MapComponents.Marker key={`${point.entity_name || ''}-${point.city || ''}-${point.state || ''}-${point.country || ''}-${i}`} position={[lat, lng]} icon={createHiringMarkerIcon(MapComponents.L, count, Boolean(point.is_fallback))}>
                  <MapComponents.Popup><div style={{ fontFamily: 'sans-serif', fontSize: 12, minWidth: 190 }}><strong>{[point.city, point.state, point.country].filter(Boolean).join(', ') || 'Unknown city'}</strong><br /><span style={{ color: '#7dd3fc' }}>{count} open job{count !== 1 ? 's' : ''}</span>{point.entity_name && <><br /><span style={{ color: '#94a3b8' }}>{point.entity_name}</span></>}</div></MapComponents.Popup>
      ...
      <div className="flex items-center justify-between gap-3 text-[10px] text-slate-500 flex-wrap shrink-0">
        <span>Dot size indicates open-job volume by city. Headquarters/state fallback points are hidden.</span>
        <span className="text-[9px] text-slate-600">{ACTIVE_BASEMAP.provider === 'arcgis' ? `ArcGIS ${ACTIVE_BASEMAP.styleLabel}` : 'Fallback basemap'}</span>
      </div>
    </div>
  );
}
