    ...
    <section className="glass-card luminous-panel relative overflow-hidden p-5">
      ...
      <div className="relative z-10 flex items-start justify-between gap-4 flex-wrap mb-4">
        <div>
          ...
          <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
            Actual server-side integration state for this entity. This is separate from whether a source happened to return jobs on the latest check.
          </p>
        </div>
        <span className="rounded-full border border-blue-400/20 bg-blue-500/8 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-blue-200">
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2.5">
        {rows.map(row => {
          ...
          const state = active && theirStackTargeted ? 'configured' : row.id === 'theirstack' && active ? 'not targeted' : 'not configured';
          return (
            <div key={row.id} className="rounded-xl border border-white/8 bg-white/[0.025] px-3.5 py-3 min-w-0">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[8px] font-semibold uppercase tracking-[0.09em] ${statusClass(state)}`}>
                  {state}
                </span>
              </div>
              <p className="mt-2 text-[9px] leading-relaxed text-slate-600 min-h-[28px]">
                {row.mode || row.detail || defaultDetail(row.id)}
              </p>
            </div>
          );
        })}
...
function statusClass(state: string) {
  if (state === 'configured') return 'border-emerald-400/20 bg-emerald-500/8 text-emerald-200';
