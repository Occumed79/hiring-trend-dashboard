...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const row of rows) {
      ...
      // A city (or better) is required. Country/state centroids and HQ fallbacks are
      // deliberately excluded. The visual layer then gives every mapped job its own
      // small point rather than collapsing an entire city into one bubble.
    ...
    if (includeMeta) return NextResponse.json({
      ...
      meta: {
        ...
        mapped_jobs: points.filter(point => !point.is_fallback).length,
        real_mapped_jobs: points.filter(point => !point.is_fallback).length,
        fallback_jobs: fallbackJobs,
        unmapped_jobs: unmappedJobs,
        point_count: points.length,
        location_count: distinctCities.size,
        map_precision: 'individual jobs at city-level or better',
